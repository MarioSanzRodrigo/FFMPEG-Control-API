 ...
 */
typedef struct ffmpeg_mlhe_enc_settings_ctx_s {
	/**
...
} ffmpeg_mlhe_enc_settings_ctx_t;

/**
 * FFmpeg's MLHE video encoder wrapper context structure.
 */
typedef struct ffmpeg_mlhe_enc_ctx_s {
	/**
...
} ffmpeg_mlhe_enc_ctx_t;

/**
 * FFmpeg's MLHE video decoder settings context structure.
 */
typedef struct ffmpeg_mlhe_dec_settings_ctx_s {
	/**
...
} ffmpeg_mlhe_dec_settings_ctx_t;

/**
 * FFmpeg's MLHE video decoder wrapper context structure.
 */
typedef struct ffmpeg_mlhe_dec_ctx_s {
	/**
...
static int ffmpeg_mlhe_enc_process_frame(proc_ctx_t *proc_ctx,
...
static int ffmpeg_mlhe_enc_settings_ctx_init(
		...
		log_ctx_t *log_ctx);
static void ffmpeg_mlhe_enc_settings_ctx_deinit(
		volatile ffmpeg_mlhe_enc_settings_ctx_t *ffmpeg_mlhe_enc_settings_ctx,
		log_ctx_t *log_ctx);

/* **** Decoder **** */

static proc_ctx_t* ffmpeg_mlhe_dec_open(const proc_if_t *proc_if,
		const char *settings_str, log_ctx_t *log_ctx, va_list arg);
static void ffmpeg_mlhe_dec_close(proc_ctx_t **ref_proc_ctx);
static int ffmpeg_mlhe_dec_process_frame(proc_ctx_t *proc_ctx,
		fifo_ctx_t* iput_fifo_ctx, fifo_ctx_t* oput_fifo_ctx);

static int ffmpeg_mlhe_dec_rest_put(proc_ctx_t *proc_ctx, const char *str);
static int ffmpeg_mlhe_dec_rest_get(proc_ctx_t *proc_ctx,
		const proc_if_rest_fmt_t rest_fmt, void **ref_reponse);

static int ffmpeg_mlhe_dec_settings_ctx_init(
		volatile ffmpeg_mlhe_dec_settings_ctx_t *ffmpeg_mlhe_dec_settings_ctx,
		log_ctx_t *log_ctx);
static void ffmpeg_mlhe_dec_settings_ctx_deinit(
		volatile ffmpeg_mlhe_dec_settings_ctx_t *ffmpeg_mlhe_dec_settings_ctx,
		log_ctx_t *log_ctx);
		...
		const char *settings_str, log_ctx_t *log_ctx, va_list arg)
{
	...
	ffmpeg_mlhe_enc_ctx= (ffmpeg_mlhe_enc_ctx_t*)calloc(1, sizeof(
			ffmpeg_mlhe_enc_ctx_t));
	CHECK_DO(ffmpeg_mlhe_enc_ctx!= NULL, goto end);
	...
	ret_code= ffmpeg_mlhe_enc_settings_ctx_init(ffmpeg_mlhe_enc_settings_ctx,
			LOG_CTX_GET());
	...
	ffmpeg_video_enc_ctx= &ffmpeg_mlhe_enc_ctx->ffmpeg_video_enc_ctx;
    ...
    if(end_code!= STAT_SUCCESS)
    	ffmpeg_mlhe_enc_close((proc_ctx_t**)&ffmpeg_mlhe_enc_ctx);
	return (proc_ctx_t*)ffmpeg_mlhe_enc_ctx;
}
...
static void ffmpeg_mlhe_enc_close(proc_ctx_t **ref_proc_ctx)
{
	...
	if((ffmpeg_mlhe_enc_ctx= (ffmpeg_mlhe_enc_ctx_t*)*ref_proc_ctx)!=
			NULL) {
		LOG_CTX_INIT(((proc_ctx_t*)ffmpeg_mlhe_enc_ctx)->log_ctx);

		/* De-initialize FFMPEG's generic video encoder context */
		ffmpeg_video_enc_ctx_deinit(&ffmpeg_mlhe_enc_ctx->ffmpeg_video_enc_ctx,
				LOG_CTX_GET());

		/* Release settings */
		ffmpeg_mlhe_enc_settings_ctx_deinit(
				&ffmpeg_mlhe_enc_ctx->ffmpeg_mlhe_enc_settings_ctx,
				LOG_CTX_GET());
...
{
	int ret_code, end_code= STAT_ERROR;
	ffmpeg_mlhe_enc_ctx_t *ffmpeg_mlhe_enc_ctx= NULL; // Do not release
	ffmpeg_video_enc_ctx_t *ffmpeg_video_enc_ctx= NULL; // Do not release
...
static int ffmpeg_mlhe_enc_rest_put(proc_ctx_t *proc_ctx, const char *str)
{
	printf("\n ---[ffmpeg_lhe.c] ffmpeg_mlhe_enc_rest_put---\n");
	int ret_code;
	ffmpeg_mlhe_enc_ctx_t *ffmpeg_mlhe_enc_ctx= NULL;
...
/**
 * Implements the proc_if_s::socket_put callback.
 * See .proc_if.h for further details.
 */
		...
		const proc_if_rest_fmt_t rest_fmt, void **ref_reponse)
{
	...
	*ref_reponse= NULL;
	...
	/* GET specific MLHE video encoder settings */
 ...
 * @param ffmpeg_mlhe_enc_settings_ctx
 * @param log_ctx
		...
		log_ctx_t *log_ctx)
{
	...
	/* Check arguments */
	CHECK_DO(ffmpeg_mlhe_enc_settings_ctx!= NULL, return STAT_ERROR);
	...
	/* Initialize specific MLHE video encoder settings */
	// Reserved for future use
		...
		log_ctx_t *log_ctx)
{
	...
	/* Check arguments */
	CHECK_DO(ffmpeg_mlhe_enc_settings_ctx!= NULL, return);
	...
	/* Release specific MLHE video encoder settings */
	// Reserved for future use
}
...
{
	int ret_code, end_code= STAT_ERROR;
	ffmpeg_mlhe_dec_ctx_t *ffmpeg_mlhe_dec_ctx= NULL;
	volatile ffmpeg_mlhe_dec_settings_ctx_t *ffmpeg_mlhe_dec_settings_ctx=
			NULL; // Do not release
	...
	/* Allocate context structure */
	ffmpeg_mlhe_dec_ctx= (ffmpeg_mlhe_dec_ctx_t*)calloc(1, sizeof(
			ffmpeg_mlhe_dec_ctx_t));
	CHECK_DO(ffmpeg_mlhe_dec_ctx!= NULL, goto end);

	/* Get settings structure */
	ffmpeg_mlhe_dec_settings_ctx=
			&ffmpeg_mlhe_dec_ctx->ffmpeg_mlhe_dec_settings_ctx;

	/* Initialize settings to defaults */
	ret_code= ffmpeg_mlhe_dec_settings_ctx_init(ffmpeg_mlhe_dec_settings_ctx,
			LOG_CTX_GET());
	CHECK_DO(ret_code== STAT_SUCCESS, goto end);

	/* Parse and put given settings */
	ret_code= ffmpeg_mlhe_dec_rest_put((proc_ctx_t*)ffmpeg_mlhe_dec_ctx,
			settings_str);
	CHECK_DO(ret_code== STAT_SUCCESS, goto end);

	/* Get generic video decoder and settings context structures */
	ffmpeg_video_dec_ctx= &ffmpeg_mlhe_dec_ctx->ffmpeg_video_dec_ctx;
    ...
    if(end_code!= STAT_SUCCESS)
    	ffmpeg_mlhe_dec_close((proc_ctx_t**)&ffmpeg_mlhe_dec_ctx);
	return (proc_ctx_t*)ffmpeg_mlhe_dec_ctx;
}
...
static void ffmpeg_mlhe_dec_close(proc_ctx_t **ref_proc_ctx)
{
	...
	if((ffmpeg_mlhe_dec_ctx= (ffmpeg_mlhe_dec_ctx_t*)*ref_proc_ctx)!= NULL) {
		LOG_CTX_INIT(((proc_ctx_t*)ffmpeg_mlhe_dec_ctx)->log_ctx);

		/* De-initialize FFMPEG's generic video decoder context */
		ffmpeg_video_dec_ctx_deinit(&ffmpeg_mlhe_dec_ctx->ffmpeg_video_dec_ctx,
				LOG_CTX_GET());

		/* Release settings */
		ffmpeg_mlhe_dec_settings_ctx_deinit(
				&ffmpeg_mlhe_dec_ctx->ffmpeg_mlhe_dec_settings_ctx,
				LOG_CTX_GET());
...
{
	int ret_code, end_code= STAT_ERROR;
	ffmpeg_mlhe_dec_ctx_t *ffmpeg_mlhe_dec_ctx= NULL; // Do not release
	ffmpeg_video_dec_ctx_t *ffmpeg_video_dec_ctx= NULL; // Do not release
	...
	ffmpeg_mlhe_dec_ctx= (ffmpeg_mlhe_dec_ctx_t*)proc_ctx;
	ffmpeg_video_dec_ctx= &ffmpeg_mlhe_dec_ctx->ffmpeg_video_dec_ctx;
...
static int ffmpeg_mlhe_dec_rest_put(proc_ctx_t *proc_ctx, const char *str)
{
	int ret_code;
	ffmpeg_mlhe_dec_ctx_t *ffmpeg_mlhe_dec_ctx= NULL;
	volatile ffmpeg_mlhe_dec_settings_ctx_t *ffmpeg_mlhe_dec_settings_ctx= NULL;
	...
	video_settings_dec_ctx=
			&ffmpeg_mlhe_dec_settings_ctx->video_settings_dec_ctx;
	...
	/* PUT specific MLHE video decoder settings */
	// Reserved for future use
		...
		const proc_if_rest_fmt_t rest_fmt, void **ref_reponse)
{
	...
	*ref_reponse= NULL;
	...
	/* GET specific MLHE video decoder settings */
	// Reserved for future use: attach to 'cjson_settings' (should be != NULL)
 ...
 * @param ffmpeg_mlhe_dec_settings_ctx
 * @param log_ctx
 * @return Status code (STAT_SUCCESS code in case of success, for other code
 * values please refer to .stat_codes.h).
 */
		...
		log_ctx_t *log_ctx)
{
	...
	/* Check arguments */
	CHECK_DO(ffmpeg_mlhe_dec_settings_ctx!= NULL, return STAT_ERROR);
	...
	/* Initialize specific MLHE video decoder settings */
	// Reserved for future use
		...
		log_ctx_t *log_ctx)
{
	...
	/* Check arguments */
	CHECK_DO(ffmpeg_mlhe_dec_settings_ctx!= NULL, return);
	...
	/* Release specific MLHE video decoder settings */
	// Reserved for future use
}
